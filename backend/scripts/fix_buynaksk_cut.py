# -*- coding: utf-8 -*-
"""Вырезать ГО Буйнакск из Буйнакского муниципального района."""
import sys
...
with engine.connect() as conn:
    overlap = conn.execute(text("""
        ...
        WHERE d1.region_id = r1.id AND d2.region_id = r2.id
          ...
          AND d1.name = 'Буйнакский муниципальный район'
          AND d2.name = 'городской округ г. Буйнакск'
...
with engine.begin() as conn:
    conn.execute(text("""
        ...
        FROM (
            ...
            WHERE r.name = :region AND d2.name = 'городской округ г. Буйнакск'
        ) city, regions r
        WHERE d.region_id = r.id AND r.name = :region
          AND d.name = 'Буйнакский муниципальный район'
    """), {"region": REGION})
    conn.execute(text("""
