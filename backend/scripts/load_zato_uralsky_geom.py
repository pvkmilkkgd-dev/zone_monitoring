"""Загрузить геометрию городского округа ЗАТО Уральский (Свердловская область) из OSM (Nominatim)."""
import os
import json
import time
import requests
...
DISTRICT_NAME = "городской округ ЗАТО Уральский"
QUERIES = [
    "ЗАТО Уральский Свердловская область Россия",
    "ZATO Uralsky Sverdlovsk Oblast Russia",
    "поселок Уральский Свердловская область",
    ...
    with engine.begin() as conn:
        ...
        for q in QUERIES:
            ...
            for item in results:
                ...
                # Проверяем, что это действительно Уральский в Свердловской области
                if "Свердловск" not in display and "Sverdlovsk" not in display:
                    continue
                if "Уральск" not in display and "Uralsky" not in display and "Uralskiy" not in display:
                    continue
                g = item.get("geojson")
                if g and g.get("type") in ("Polygon", "MultiPolygon"):
                    geojson = g
                    print(f"  Найдено в Nominatim по запросу: {q}")
                    break
            if geojson:
                break
