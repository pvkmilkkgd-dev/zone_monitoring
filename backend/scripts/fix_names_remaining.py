"""Fix remaining short district names using known OSM region IDs."""
...
# Known OSM relation IDs for problematic regions
REGION_OSM_IDS = {
...
def get_districts_by_region_id(osm_id):
    """Get admin_level 6 district names from Overpass using region OSM ID."""
...
# Get remaining districts with short names
...
print(f"Remaining: {len(rows)} districts across {len(by_region)} regions\n")
...
for region_name, districts in sorted(by_region.items()):
    ...
    if not osm_id:
        print(f"=== {region_name} - no OSM ID configured, skipping {len(districts)} ===")
        total_failed += len(districts)
        continue
    
    print(f"\n=== {region_name} (OSM R{osm_id}, {len(districts)} to fix) ===")
    
    osm_names = get_districts_by_region_id(osm_id)
    time.sleep(2)
    
    if not osm_names:
        print(f"  No OSM names found")
        total_failed += len(districts)
        continue
    
    print(f"  Got {len(osm_names)} OSM names")
